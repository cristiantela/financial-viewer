  ...
  methods: {
    ...
    spent(payments) {
      return -this.sumArray(
        this.notSuspendedPayments(payments)
          .map((payment) => Number(payment.value) || 0)
    ...
    groupTags(payments) {
      ...
      this.notSuspendedPayments(this.paymentsInline(payments))
      ...
      return tags.filter((tag) => !(tag.name === "others" && tag.value === 0));
    },
  },
};
</script>
